            ...
            <div className="p-4 border-b">
              <div className="relative">
                ...
                <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="Search conversations..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </div>
            </div>
            <div className="flex-1 overflow-y-auto">
              {filtered.length > 0 ? filtered.map((c) => (
                <button key={c.id} onClick={() => setSelectedChat(c.id)}
                  className={`w-full p-4 border-b border-gray-100 hover:bg-gray-50 text-left transition-colors ${selectedChat === c.id ? 'bg-indigo-50' : ''}`}>
                  <div className="flex justify-between items-start mb-1">
                    <span className="font-medium">{c.other_name}</span>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    {c.unread > 0 && <span className="ml-2 bg-indigo-600 text-white text-xs px-2 py-1 rounded-full">{c.unread}</span>}
                  </div>
                </button>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {messages.map((m) => (
                  <div key={m.id} className={`flex ${m.sender_id === myId ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-xs px-4 py-2 rounded-lg ${m.sender_id === myId ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-800'}`}>
                      <p>{m.content}</p>
                      <p className={`text-xs mt-1 ${m.sender_id === myId ? 'text-indigo-100' : 'text-gray-500'}`}>
              ...
              <form onSubmit={handleSend} className="p-4 border-t">
                <div className="flex gap-2">
                  <input type="text" value={messageText} onChange={(e) => setMessageText(e.target.value)}
                    placeholder="Type a message..." className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                  <button type="submit" className="bg-indigo-600 text-white px-6 py-2 rounded-lg hover:bg-indigo-700 transition-colors"><Send className="w-5 h-5" /></button>
                </div>
              </form>
